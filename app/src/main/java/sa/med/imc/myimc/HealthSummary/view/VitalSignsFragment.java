...
public class VitalSignsFragment extends Fragment implements HealthViews, BottomFilterReadingDialog.BottomDialogListener {
    ...
    void callAPI() {
        presenter.callGetAllReadings(SharedPreferencesUtils.getInstance(getActivity()).getValue(Constants.KEY_ACCESS_TOKEN, ""),
                SharedPreferencesUtils.getInstance(getActivity()).getValue(Constants.KEY_MRN, ""), booking_id, from, to, Constants.RECORD_SET, String.valueOf(page),
        ...
        if (response.getReadingModels() != null) {
            ...
            for (int i = 0; i < response.getReadingModels().size(); i++) {
                ...
                if (model.getHeight() != null || model.getWeight() != null ||
                        model.getBodyTempture() != null || model.getPluseRate() != null ||
