...
public class PhysicianAdapter extends RecyclerView.Adapter<PhysicianAdapter.Viewholder> {
    List<PhysicianResponse.Physician> list;
    List<PhysicianResponse.Physician> All_list;
    ...
    public PhysicianAdapter(Context context, List<PhysicianResponse.Physician> list) {
        this.context = context;
        this.list = list;
        lang = SharedPreferencesUtils.getInstance(context).getValue(Constants.KEY_LANGUAGE, Constants.ENGLISH);
    }

    public PhysicianAdapter(Context context, List<PhysicianResponse.Physician> list, String yes) {
        this.context = context;
        this.list = list;
        this.yes = "1";
        lang = SharedPreferencesUtils.getInstance(context).getValue(Constants.KEY_LANGUAGE, Constants.ENGLISH);
    ...
    public void onBindViewHolder(@NonNull Viewholder holder, final int position) {
        PhysicianResponse.Physician physician = list.get(position);

        if (yes.equals("1") && physician.getService() != null && physician.getSessionId() == null)
