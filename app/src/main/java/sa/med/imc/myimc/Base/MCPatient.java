...
public class MCPatient {
    ...
    private static MCPatient instance = new MCPatient();
...
//
    ...
    public static synchronized MCPatient getInstance() {
        if(instance.get() == null){
            if(SharedPreferencesUtils.getInstance(ImcApplication.getInstance()).getValue(Constants.KEY_PATIENT_ID,-1)!=-1){
              //4 jan
                instance.setCurrent(Patient.get(SharedPreferencesUtils.getInstance(ImcApplication.getInstance()).getValue(Constants.KEY_PATIENT_ID,-1)));
            }
...
//4 jan
    ...
    public synchronized Patient get() {
        ...
        if (current != null) {
            ...
            int id = SharedPreferencesUtils.getInstance(ImcApplication.getInstance()).getValue(Constants.KEY_PATIENT_ID,-1);

            if (id == -1)
                return null;

            current = Patient.get(id);
