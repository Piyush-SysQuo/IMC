...
public class ReportsImpl implements ReportsPresenter {
    ...
    @Override
    public void callGetAllLabReports(String token, String mrn_number, String searc, String episodeId, String item_set, String page, String from, String to, int hosp) {
        ...
        Call<ResponseBody> xxx = webService.fetchLabReports(body);
                    ...
                    if (responseBody != null) {
                        ...
                        try {
                            ...
                            if (object1.has("code")) {
                                ...
                                LabReportResponse response1 = new Gson().fromJson(response11, LabReportResponse.class);
                                if (response1.getStatus().equalsIgnoreCase("true")) {
//                            if (searc.length() > 0)
//                                views.onGetSearchLabReports(response1);
//                            else {
                                    if (searc.length() == 0)
                                        if (response1.getLabReports() != null)
                                            for (int i = 0; i < response1.getLabReports().size(); i++) {
                                                LabReport data1 = response1.getLabReports().get(i);
                                                data1.setMRN(mrn_number);
                                                db.labReportDataDao().saveLabReportData(data1);
                                            }

                                    views.onGetLabReports(response1);
//                            }
                                } else
            ...
            public void onFailure(Call<ResponseBody> call, Throwable t) {
                ...
                if (t instanceof SocketTimeoutException) {//ClassNotFoundException
                    ...
                    List<LabReport> labReports = db.labReportDataDao().loadLabReportData(mrn_number);

                    if (labReports != null && labReports.size() > 0) {
                        message = activity.getString(R.string.network_content);
                        views.onFail(message);

                        LabReportResponse response = new LabReportResponse();
                        response.setLabReports(labReports);
                        response.setStatus("true");
                        response.setMessage("no_net");
                        response.setTotal_items(String.valueOf(labReports.size()));
                        views.onGetLabReports(response);
                    } else {
                        views.onNoInternet();
                    }
                } else {
...
//                    views.onFail(message);
    ...
    @Override
    public void callGenerateCardioPdf(String token, String mr_number, String attendanceId, String reportType) {
        views.showLoading();
        WebService webService = ServiceGenerator.createService(WebService.class, "Bearer " + token);
        JSONObject object = new JSONObject();
        try {
...
//                    views.onFail(message);
    ...
    public void callGetAllLabReportsMedicus(String token, String mrn_number, String searc, String episodeId, String item_set, String page, String from, String to, int hosp) {
        ...
        RequestBody body = RequestBody.create(okhttp3.MediaType.parse("application/json; charset=utf-8"), (object).toString());
        WebService webService = ServiceGenerator.createService(WebService.class, "Bearer " + token);
        Call<ResponseBody> xxx = webService.fetchLabReportsMedicus(body);
        xxx.enqueue(new Callback<ResponseBody>() {
            @Override
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
...
//                                                    labReportsParentMedicus.setFamilyName(familyName);
    ...
    public void callGetPin(String token, String mr_number) {
        ...
        WebService webService = ServiceGenerator.createService(WebService.class, "Bearer " + token);
        Call<PinResponse> xxx = webService.getPinForMedicus(body);
