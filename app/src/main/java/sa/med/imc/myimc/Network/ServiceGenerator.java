...
public class ServiceGenerator {
    ...
    public static <S> S createService(Class<S> serviceClass) {
        return createService(serviceClass, BuildConfig.BASE_URL, new MyOkHttpInterceptor(""));
    }

    public static <S> S createService(Class<S> serviceClass, String baseUrl, String no) {
        return createService(serviceClass, baseUrl, new MyOkHttpInterceptor(no));
    }

    public static <S> S createService(Class<S> serviceClass, String token) {
        return createService(serviceClass, BuildConfig.BASE_URL, new MyOkHttpInterceptor(token));
    }
    public static <S> S createPharmacyService(Class<S> serviceClass, String token) {
        return createService(serviceClass, BuildConfig.BASE_URL_PHARMACY, new MyOkHttpInterceptor(token));
    }


    public static <S> S createService1(Class<S> serviceClass, String token) {
        return createService(serviceClass, WebUrl.BASE_URL_JD, new MyOkHttpInterceptor(token));
    }
